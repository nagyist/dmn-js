...
Viewer.prototype.importXML = function(xml, done) {
  ...
  this.moddle.fromXML(xml, 'dmn:Definitions', function(err, definitions, context) {
    ...
    console.log('imported definitions', definitions);

    self.importDefinitions(definitions, 0, function(err, importWarnings) {
...
Viewer.prototype.getDecisions = function() {
  return this.definitions && this.definitions.decision
};

Viewer.prototype.showDecision = function(decision, done) {
  if(!this.definitions) {
    throw "Definitions not parsed yet";
  }

  // get index of the decision
  var idx = this.definitions.decision.indexOf(decision);
  if(idx === -1) {
    throw "Unknown decision object";
  }

  // import the definition with the given index
  this.importDefinitions(this.definitions, idx, function(err, importWarnings) {
    var allWarnings = [].concat(parseWarnings, importWarnings || []);
...
Viewer.prototype.importDefinitions = function(definitions, decisionIdx, done) {
  ...
  try {
    ...
    Importer.importDmnTable(this, definitions, decisionIdx, done);
  } catch (e) {
    done(e);
  }
};
